...
function caesar(str, key = 1) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (currentChar >= 97 && currentChar <= 122) {
      if (currentChar + key > 122) result += String.fromCharCode(97 + (currentChar + key - 123));
      else result += String.fromCharCode(currentChar + key);
    ...
    if (currentChar >= 65 && currentChar <= 90) {
      if (currentChar + key > 90) result += String.fromCharCode(65 + (currentChar + key - 91));
      else result += String.fromCharCode(currentChar + key);
